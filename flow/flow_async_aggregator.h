...
namespace detail {
    ...
    struct flow_when_all_state {
        ...
        flow_when_all_state() noexcept
                : data{Ts(error_tag, typename Ts::error_type{})...}, fired(0), failed(sizeof...(Ts)), controllers{} {
        }
            ...
            void emplace(value_type &&data_) noexcept {
                ...
                UNLIKELY_IF (state.fired.get().fetch_sub(detail::epoch, std::memory_order_release) ==
                    ...
                    LIKELY_IF (i == sizeof...(Ts)) {
                        ...
                        owner_raw->resume(
                                result_type(error_tag, async_any_failed_error<flow_async_agg_err_t>::make(i)));
                    }
                }
            }
        };
    ...
    struct flow_when_all_awaitable final :
        ...
        int submit(std::index_sequence<idx...>) {
            ...
            (void) swallow{
                    0,
                    (ok && ((ok = !launch<idx>()) && (state_.failed.get().load(std::memory_order_relaxed) == N)), 0)...
            };
            return ok ? 0 : -1;
        }
    ...
    struct flow_when_all_fast_state {
        ...
        flow_when_all_fast_state() noexcept
                : data{Ts(error_tag, typename Ts::error_type{})...}, fired(0), failed(sizeof...(Ts)) {
        }
        ...
        public:
            ...
            void emplace(value_type &&data_) noexcept {
                ...
                UNLIKELY_IF (e.has_error()) {
                    size_t exp = sizeof...(Ts);
                    state.failed.get().compare_exchange_strong(exp, I, std::memory_order_acq_rel,
    ...
    struct flow_when_any_state {
        ...
        flow_when_any_state() noexcept
                : data{Ts(error_tag, typename Ts::error_type{})...}, fired(0), winner(sizeof...(Ts)), controllers{} {
        }
        ...
        int submit(std::index_sequence<idx...>) {
            ...
            (void) swallow{
                    0, (keep_launching && ((count += !launch<idx>()),
                                            keep_launching = (state_.winner.get().load(std::memory_order_acquire) ==
                                                              N)), 0)...
            };
            return count;
        }
