...
struct executor_env {
    ...
    executor_env()
        ...
        while (started.load(std::memory_order_acquire) == 0) {
            ...
            if (std::chrono::duration_cast<std::chrono::seconds>(elapsed).count() > 2) {
                std::printf("[FAIL] simple_executor start timeout\n");
                std::abort();
            }
        }
    }
    ...
    ~executor_env() {
        ...
        if (!shutdown_ok) {
            std::printf("[FAIL] simple_executor shutdown timeout\n");
            std::abort();
        }
...
void print_stat(const test_stat& s, const config_t& cfg, long long total_ms) {
    std::printf("[flow multiflow cancel stress / simple_executor]\n");
...
int main(int argc, char** argv) {
    ...
    auto bp = make_blueprint<int>()
        ...
        | via(&env.ex)
        | await<delayed_plus_one_awaitable>(&env.ex)
    ...
    if (failed == 0) {
        std::printf("[PASS] multiflow cancel stress (simple_executor) passed\n");
        return 0;
    }

    std::printf("[FAIL] multiflow cancel stress (simple_executor) found %d issues\n", failed);
    return 1;
}
