...
int test_full_chain_success() {
    ...
    auto bp = make_blueprint<int>()
        ...
        | then([](out_t&& in) noexcept -> out_t {
            if (!in.has_value()) {
                return out_t(error_tag, std::move(in).error());
            }
    ...
    int failed = 0;
    check(wait_done(obs, 1000), "full_chain wait done", failed);
    check(obs.has_value.load(std::memory_order_acquire) == 1, "full_chain has value", failed);
    check(obs.value.load(std::memory_order_acquire) == 13, "full_chain value == 13", failed);
...
int test_on_error_recover() {
    ...
    int failed = 0;
    check(wait_done(obs, 1000), "on_error wait done", failed);
    check(obs.has_value.load(std::memory_order_acquire) == 1, "on_error recovered", failed);
    check(obs.value.load(std::memory_order_acquire) == 88, "on_error value == 88", failed);
...
int test_catch_exception_recover() {
    ...
    int failed = 0;
    check(wait_done(obs, 1000), "catch_exception wait done", failed);
    check(obs.has_value.load(std::memory_order_acquire) == 1, "catch_exception recovered", failed);
    check(obs.value.load(std::memory_order_acquire) == 77, "catch_exception value == 77", failed);
...
int test_when_all() {
    ...
    check(obs.has_value.load(std::memory_order_acquire) == 1, "when_all has value", failed);
    check(obs.value.load(std::memory_order_acquire) == 35, "when_all value == 35", failed);
...
int test_when_any() {
    ...
    auto bp = await_when_any(
        &env.ex,
        [](size_t, int x) noexcept { return out_t(value_tag, x); },
        [](flow_async_agg_err_t e) noexcept { return out_t(error_tag, std::move(e)); },
        p1,
        p2)
        | end();
    ...
    check(obs.has_value.load(std::memory_order_acquire) == 1, "when_any has value", failed);
    int value = obs.value.load(std::memory_order_acquire);
    check(value == 102 || value == 203, "when_any winner value in {102,203}", failed);
...
int main() {
    ...
    if (failed == 0) {
        std::printf("[PASS] fullchain smoke passed\n");
        return 0;
    }

    std::printf("[FAIL] fullchain smoke failed with %d failures\n", failed);
    return 1;
}
